package com.icwd.apigateway.controller;
...
import com.icwd.apigateway.response.AuthResponse;
...
public class AuthController {
	...
	public ResponseEntity<AuthResponse> login(@RegisteredOAuth2AuthorizedClient("okta") OAuth2AuthorizedClient client,
		...
		List<String> authorities = user.getAuthorities().stream().map(grantedAtuthority -> {
			return grantedAtuthority.getAuthority();
